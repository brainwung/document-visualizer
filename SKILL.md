...
1. **确认输入和决策状态。**
   ...
   - 多风格同时输出只用于风格选择，不用于交付成品。默认不要同时制作 3 个完整 HTML；如果需要比较风格，只生成同一内容的轻量预览图或预览级 HTML，等用户选定后再制作一个完整版本。

2. **读取源材料。**
   - 识别文件类型和可用内容。
   - 提取标题层级、章节、表格、图示、日期、人物、实体、指标、决策、风险和待办。
   - 如果源材料是图片结构图或流程图，先提取可识别的节点、连线、方向、分组、层级和疑似文字。
   - 如果源材料是图片化的数据报告、架构图、流程图或信息图，必须先把它还原成信息模型，再重新选择适合的版式和风格；不要把图片当作要临摹的视觉稿。
...
4. **推荐输出格式和风格。**
   ...
   - 图片资料的原始配色、卡片形态和排版节奏只作为信息识别线索，不作为默认视觉方案；除非用户明确要求复刻，否则必须基于资料类型重新选择风格。
   ...
   - 如果用户明确要求“每种风格都看一下”，输出应标注为“风格预览”，内容密度要低于正式成品，且三种预览必须使用同一份源内容和同一信息层级，便于比较视觉差异。
...
5. **制作输出。**
   ...
   - 正式制作阶段只针对一个已确认风格打磨完整 HTML/SVG。不要把多个风格并行推进到成品级，否则会牺牲每个版本的结构、样式和验证质量。
